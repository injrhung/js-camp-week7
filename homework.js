...
function validateCartQuantity(quantity) {
  ...
  if(!Number.isInteger(quantity)){
    return {
      isValid: false, error:"數量必須是正整數" }
  }
  if(quantity < 1){
    return {
      isValid: false, error:"數量不可小於 1" }
  }
  if(quantity > 99){
    return {
      isValid: false, error:"數量不可大於 99" }
  } 
...
function generateCartItemId() {
  ...
  const timestampPart = Date.now().toString(36);
  const randomPart = Math.random().toString(36).slice(2, 10);
  return `CART-${timestampPart}${randomPart}`;  
...
1. _fetch需手動執行，axious自動轉換______

2. _axios只要狀態碼不在 2xx 範圍內就會拋出錯誤，fetch則不會______

3. _fetch較繁鎖 axios 有更簡潔的語法____
*/
